...
export default function ServiciosTab({
  ...
  //
  // Antes esta función filtraba por dentro con `itemSearch`. Ahora el
  // filtrado vive en la pieza (AgregadorDeItems), así que acá solo se
  // arma la lista ORDENADA por sección, con el nombre de la sección
  // como `group`.
  //
  ...
  // diga "postre". Sale gratis porque la pieza busca en la etiqueta Y
  // en el grupo — exactamente los dos textos que se pasaban antes.
  const opcionesDe = (g: any) => {
    ...
    // Sin secciones: lista plana, SIN ningún rótulo (si se pusiera
    // "Sin sección" aparecería un encabezado donde hoy no hay nada).
  ...
  // El botón manual OBEDECE (13-08, pillada de Felipe): guarda aunque
  // la foto no muestre diferencias, y confirma con la hora. Un humano
  // que aprieta Guardar espera un guardado, no un criterio silencioso —
  // apretaba el botón y "no pasaba nada" porque nada había cambiado.
  // El costo de un guardado de más es un viaje idéntico al de siempre;
  // el costo de la mudez era desconfiar de la pantalla entera.
  ...
  const recursosEvento = useQuery({
    ...recursosQueryOpts(
      company?.id ? Number(company.id) : 0,
      String(quote.id),
    ),
                    ...
                    <div className="py-1.5 border-t border-gray-100">
                      {addingToGroup === gi ? (
                        <div className="flex items-center gap-2">
                          {/* Migrado a la pieza de la casa (14-08). Eran 134
                              líneas escritas a mano. Ojo con los DOS niveles:
                              `addingToGroup` decide si existe esta fila con su
                              botón Cancelar, y `openItemPicker` si el panel
                              está desplegado. Cerrar el panel NO debe hacer
                              desaparecer la fila — por eso onAbiertoChange
                              solo toca openItemPicker. */}
                          <div className="relative dropdown-container flex-1">
                            <AgregadorDeItems
                              opciones={opcionesDe(g)}
                              onAgregar={(codigo) => addToGroup(gi, codigo)}
                              abierto={openItemPicker === gi}
                              onAbiertoChange={(v) =>
                                setOpenItemPicker(v ? gi : null)
                              }
                              placeholder={`Seleccionar servicio de ${g.category}…`}
            ...
            <div className="mt-8 pt-6 border-t border-gray-200">
              ...
              panel que queda abierto para agregar varios seguidos. */}
                <div className="flex items-center gap-2 py-2 border-b border-gray-100">
                  {/* Migrado a la pieza de la casa (14-08). Eran 93 líneas.
                      Los rótulos de sección solo salen si el catálogo de
                      fijos TIENE secciones: si no, lista plana. Sin ese
                      candado aparecería un "Sin sección" arriba de todo
                      donde hoy no hay nada. */}
                  <div className="relative dropdown-container flex-1">
                    <AgregadorDeItems
                      opciones={fixedCatalog
                      ...
                      onAgregar={(codigo) => addFixedSvc(codigo)}
                      abierto={openFixedPicker}
                      onAbiertoChange={setOpenFixedPicker}
              ...
              <div className="bg-white rounded-xl shadow p-4">
                ...
                {!margenEvento ? (
                  ...
                  (() => {
                    ...
                    const pctSinDesc =
                      ventaSinDesc > 0
                        ? (margenSinDesc / ventaSinDesc) * 100
                        : 0;
                    const fmt = (n: number) =>
          ...
          cotizador: Resumen → Descuento → Margen → Observaciones). */}
            <div className="bg-white rounded-xl shadow p-4">
              ...
              <p className="text-xs text-gray-400 mt-3">
                Editable: adultos y niños, servicios y cantidades, descuento (%
                o $), propina equipo y comentarios. Cada servicio multiplica por
                las personas de SU audiencia. Al guardar, si cambia el total, el
                plan de pagos se ajusta automáticamente.
              </p>
            </div>
          </div>
        </div>
