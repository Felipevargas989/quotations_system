...
| `POST /users/signup` (pública) y `POST /super-admin/suscription` (manual de Felipe desde el 16-09) | `UsersController.signup` → `UsersService.signup`; `SuperAdminController.createSuscription` (+`assertSuperAdmin`) | `SuperAdminService.createSuscription` → `NEW_ACCOUNT` y `SUPER_ADMIN_NEW_COMPANY` | `NewUserRegisterForm` (`/register`): el alta por cuenta propia volvió el 16-09-2026 | `signup`: `@Public` + `@Throttle` 10/min y DTO endurecido; `suscription`: solo super-admin |
...
| `NEW_ACCOUNT`: "Bienvenido a Eventia" | `SuperAdminService.createSuscription` | `admin_email` del registro | `newAccount.ts` (con `NewAccountParams` desde el 16-09: nombre de la empresa y vencimiento de la prueba; el botón lleva a `/login`, y el nombre se escapa porque la puerta es pública) | Ninguna | Vive: lo dispara cada registro de `/register` |
