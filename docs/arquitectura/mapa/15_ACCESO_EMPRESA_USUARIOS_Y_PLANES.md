...
- **Planes a medio camino** (medido el 11-09-2026): precio ($10.000) y enlace de Mercado Pago escritos a fuego en `Plans.tsx`; `is_premium` solo decide el banner; nada vence la "prueba de 7 días"; `companies.is_active` no lo lee nadie (ni `AuthGuard`). `docs/mapa-programacion-planes.md` (24-07) describe 3 planes, columnas `plan`/`subscription_status`, webhook y candado por plan: nada de eso existe en el código.
  - **Corregido en parte el 14-09-2026** (§5.8): el candado por plan **sí existe** en el motor, con las columnas `plan` y `estado_plan` de la migración 112 y un reloj que vence las pruebas de 7 días. Lo que sigue pendiente: el cobro (paso 5 del roadmap), el precio y el enlace escritos a fuego en `Plans.tsx`, `is_premium` sin nadie que lo mire de verdad, `is_active` sin lector, y la pantalla, que todavía no sabe nada de derechos. La columna que la 112 trae se llama `estado_plan`, no `subscription_status`.
...
**La Torre de Control es por donde Felipe vende a mano** mientras el cobro automático no exista. `PATCH /super-admin/companies/:id` ahora acepta `plan`, `estado_plan` y `prueba_vence` con una clase DTO de verdad (`ActualizarEmpresaDto`, en `super-admin/dto/cambiar-plan.dto.ts`), estampa `plan_cambiado_en` y —esto es lo importante— **olvida la memoria de los derechos de la empresa y el perfil de todos sus usuarios**: sin eso, el cliente que acaba de pagar seguiría viendo su plan viejo hasta una hora. De dónde salen esos datos en la pantalla: `GET /super-admin/companies` entrega la fila completa de la empresa (`select('*')`), con plan, estado, vencimiento y módulos propios incluidos. `GET /super-admin/torre` **todavía no**: su consulta (`SuperAdminRepository.getTorreBase`) ya lee las cuatro columnas de `companies`, pero `getTorre` solo las usa para el nombre de la empresa y las tarjetas, y su respuesta (`TorreResponse`) sigue siendo `usuarios` + `tarjetas`. Medido el 14-09-2026.
...
**Lo que todavía no está** (medido el 14-09-2026): el candado vive **solo en el motor**. La app no sabe nada de derechos — un grep de `derechos`, `SIN_DERECHO`, `SIN_CUPO` y `PLAN_BLOQUEADO` en `frontend/src` no encuentra nada, y `permissions.ts` sigue conociendo únicamente los módulos propios (`SECTION_MODULO`, `tieneModulo`). Es decir: hoy una empresa de Cotiza ve el menú completo y recibe el 403 al tocar. Es el mismo problema que la zona de riesgo 4 de este mapa ("no se muestra un control que el servidor rechaza"), y es el trabajo que falta para cerrar el paso 3.2 en la pantalla.
