...
|---|---|---|---|---|
| `GET /quotations` | `QuotationsController.findAll` | `QuotationsService.findAll` → `QuotationsRepository.findAll` (lista liviana `COLUMNAS_LISTA`, sin `items`) | `getQuotations` ← `QuotationsPage` (el tablero: llaves `embudo-y-rechazadas`, `cerradas-busqueda` y `requirements`), `NegocioPage` (la ficha, llave `ficha-lista`) y `RequestsPage`. La misma función la llaman además `Calendar`, `DashboardPage`, `NewAccount` (en `DashboardPage`) y las pestañas `FichasTab` y `SemanaTab` de Personal | Autenticado, sin `@Roles` |
| `GET /quotations/:id` | `QuotationsController.findOne` | `QuotationsService.findOne` → `QuotationsRepository.findOne` (`select *`) | `getQuotationById` ← `NegocioPage` (detalle de la ficha) y `QuotationsPage` (visor PDF del tablero). Fuera de este mapa la llaman `QuotationForm`, `PostVentaPage`, `ClientDetailPage`, `Calendar`, `ResumenDelDia`, `PublicSurvey` y el componente `PaymentPlanEditor` | **`@Public`** (lo usa la encuesta pública; TODO en el controller) |
...
| `GET /quotations/check-conflicts` | `QuotationsController.checkConflictsWithExistingQuotations` | `QuotationsService.checkConflictsWithExistingQuotations` | `checkConflictsWithExistingQuotations` ← el componente `EventoCajitas` (con `exclude_id`), montado en `NegocioPage` y `PostVentaPage`; vía el hook `useDateAvailability`, también `QuotationForm` (mapa 01) y `RequestForm` (en `RequestsPage`) | Autenticado |
| `POST /quotations` | `QuotationsController.create` | `QuotationsService.create` | `createQuotation` ← `RequestForm`, el formulario de requerimientos que monta `RequestsPage` (desde el cotizador, `QuotationForm`, es el mapa 01) | Autenticado; recepción solo `request_type = requerimiento` |
...
| `GET /quotation-followups/map` | `QuotationFollowupsController.map` | `QuotationFollowupsService.mapByCompany` | `getFollowupsMap` ← `QuotationsPage` (semáforo del tablero) y `PostVentaPage` (aviso ámbar) | `RECEPTION_AND_UP` (en la clase) |
| `GET /quotation-followups/by-quotation/:quotationId` | `QuotationFollowupsController.findByQuotation` | `QuotationFollowupsService.findByQuotation` | `getFollowupsByQuotation` ← `HiloSeguimiento`, montado en `NegocioPage` y `PostVentaPage` | `RECEPTION_AND_UP` |
| `POST /quotation-followups` | `QuotationFollowupsController.create` | `QuotationFollowupsService.create` | `createFollowup` ← `HiloSeguimiento.guardar` (en `NegocioPage` y `PostVentaPage`) y, para el comentario de `MotivoPerdida` al rechazar, `NegocioPage` y `QuotationsPage`. Además lo usa por dentro `EnvioCotizacionService` | `RECEPTION_AND_UP` |
| `PATCH /quotation-followups/:id` | `QuotationFollowupsController.update` | `QuotationFollowupsService.update` | `updateFollowup` ← `HiloSeguimiento` (editar nota y botón "Listo"), montado en `NegocioPage` y `PostVentaPage`; ninguna otra pantalla | `RECEPTION_AND_UP` + autoría, salvo marcar cumplido |
| `DELETE /quotation-followups/:id` | `QuotationFollowupsController.remove` | `QuotationFollowupsService.remove` | `deleteFollowup` ← `HiloSeguimiento`, montado en `NegocioPage` y `PostVentaPage`; ninguna otra pantalla | `RECEPTION_AND_UP` + autoría |
| `GET /event-documents?quotationId=`, `POST /event-documents`, `DELETE /event-documents/:id` | `EventDocumentsController` (`api-rest/src/quotations/event-documents.controller.ts`) | `EventDocumentsRepository` (en el mismo archivo) | `getDocumentsByQuotation`, `addDocument` y `deleteDocument` (`documents.service.ts`, categoría `comercial`) ← `AdjuntosComerciales`, montado en `NegocioPage` y `PostVentaPage`; la pestaña Documentos de `PostVentaPage` usa las mismas funciones con las otras categorías | `OPERATIONS_AND_UP` (en la clase) |
