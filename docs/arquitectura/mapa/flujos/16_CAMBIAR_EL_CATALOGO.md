...
23. **Borrar un menú:** `useServiceGroups.removeGroup` → `DELETE /service-groups/:id` → `ServiceGroupsService.remove` → `ServiceGroupsRepository.removeGroup(id)`, **sin empresa**. La base arrastra `service_group_items` (migración 1) y `service_group_collection_items` (migración 3): el menú desaparece de los paquetes que lo incluían. La app solo refresca `["serviceGroups"]`, no `["serviceGroupCollections"]`.
24. **Borrar un paquete:** `useServiceGroupCollections.removeCollection` → `DELETE /service-group-collections/:id` → `ServiceGroupCollectionsService.remove` → `ServiceGroupCollectionsRepository.removeCollection(id)`, sin empresa. La base arrastra sus tres tablas hijas.
...
sequenceDiagram
    ...
    API->>DB: UPDATE variable_services (solo por id)
...
|---|---|---|---|
| `variable_services` | `name`, `price`, `no_cost` | 3 | `ServicesRepository.updateVariableService` (solo por id) |
...
| `variable_service_categories` | filas nuevas (`sort_order` al final) y filas borradas | 4 | `ServicesRepository.insertServiceCategoryLink` y `deleteServiceCategoryLink` (este último sin empresa) |
...
| `fixed_services` | `name`, `calculation_type`, `price`, `min_price`, `max_price`, `price_per_person`, `no_cost` | 8 | `ServicesRepository.updateFixedService` (solo por id) |
...
| `service_groups` | fila completa (borrado) | 23 | `ServiceGroupsRepository.removeGroup` (sin empresa) |
| `service_group_items` | filas en cascada | 23 | la base (migración 1) |
| `service_group_collections` | fila completa (borrado) | 24 | `ServiceGroupCollectionsRepository.removeCollection` (sin empresa) |
...
13. **Si tocas** `ServicesRepository.updateVariableService`, `updateFixedService`, `getLinksForService`, `deleteServiceCategoryLink`, `ServiceGroupsRepository.removeGroup` o `ServiceGroupCollectionsRepository.removeCollection`, **pasa** que sigue abierta la puerta para cambiar o borrar datos de otra empresa por id, **porque** filtran solo por `id` y Supabase corre con la llave service-role (CLAUDE.md). Además, varios devuelven la respuesta cruda de Supabase sin revisar `error`, y una falla puede llegar como 200. Es el mismo tipo de hoyo que se cerró el 13-08 en `DELETE /services/*`. Evidencia: esos métodos; mapa 05, R5 y R9.
...
| `api-rest/src/service-groups/tests/service-groups.service.spec.ts` | `ServiceGroupsService.rename`: recorta y pasa la empresa, 409 en castellano, 404 si es ajeno (3 pruebas) | Sí |
...
- Aislamiento por empresa de `updateVariableService`, `updateFixedService`, `removeGroup` y `removeCollection`.
