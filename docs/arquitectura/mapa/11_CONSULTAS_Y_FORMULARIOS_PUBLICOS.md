...
| Método y ruta | Controller y método | Service | Quién lo llama desde la app | Roles o @Public |
|---|---|---|---|---|
| `POST /quotations/public/:company_id` | `QuotationsController.createPublic` | `QuotationsService.createPublic`: bifurca con `ConsultasService.embudoPara` y `ConsultasService.registrar`. **Con red desde el 22-09-2026:** si el guardado falla por la base (no por validación 4xx), `RescateDelFormularioService.rescatar` manda el lead COMPLETO por Resend directo a `SUPER_ADMIN_EMAILS` (sin pasar por `EmailService`, que necesita la base) y responde `{tipo:'rescatada'}`, así el visitante ve el mismo "gracias"; si ni el correo sale, el error llega al visitante como antes | `createQuotationPublic` (`frontend/src/services/quotations.service.ts`) ← `CreateQuotationPublic` | `@Public()` + `@Throttle` 10 por minuto |
...
3. `createQuotationPublic` → `POST /quotations/public/:company_id` → `CreateQuotationPublicDto` (correo y teléfono obligatorios) → `QuotationsService.createPublic`. **La red (22-09-2026, incidente 71):** el controller envuelve todo en try/catch; un `HttpException` 4xx (validación, cupo) pasa tal cual; cualquier otro fallo (tope de tiempo, PGRST003, red) dispara `RescateDelFormularioService.rescatar` → correo `🚨 Cotización rescatada: …` con correo, teléfono, nombre, organización, evento, fecha, personas, presupuesto y observaciones SIN tapar, `replyTo` al visitante; con `EMAILS_SILENCED=1` no manda pero deja los datos completos en el registro. Nació porque el 22-09 el Hospital San Agustín de Florida cotizó tres veces con la base caída y sus datos de contacto se perdieron (el registro los tapa con `logSafe`). Prueba: `quotations/tests/rescate-del-formulario.spec.ts`.
...
4. Relee `consulta_config` (`repo.config`) y llama `enviarBrochure`: marca con `marcaDesdeFila`, filtra los brochures cuya ruta no empieza con `c{empresa}/`, descarga cada PDF del balde, cambia `{nombre}` por el primer nombre (o usa `TEXTO_DE_LA_CASA`), escapa cada párrafo con `escaparHtml`, arma `plantillaCampana` sin link de baja ni botón de cotizar y envía por **Resend directo**: `from` `{empresa} <hola@eventi-app.com>`, asunto "{empresa}: valores para tu {tipo}", `replyTo` solo si la empresa lo tiene y, desde el 16-09-2026, **copia oculta a ese mismo buzón** (la regla de "Enviar cotización", sin duplicar si quien consulta es ese buzón). Felipe lo pidió porque contacto@ recibía las respuestas del cliente pero no veía el correo que había salido.
