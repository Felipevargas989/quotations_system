...
| [22_AISLAMIENTO_ENTRE_EMPRESAS.md](22_AISLAMIENTO_ENTRE_EMPRESAS.md) | Inventario de las puertas donde hoy una empresa puede leer, cambiar o borrar datos de otra (31 confirmadas), sus dos causas y el plan en 5 sprints | Verificado | (documento transversal, sin tablas propias) | El sprint 1 (catálogo) está **hecho solo en el laboratorio** (rama `pruebas`, commit `8266ba1`); no está en producción, espera el "a producción" de Felipe. Los sprints 2 a 5 (pagos, cotización ajena, fichas de empresa/usuarios, puertas públicas) siguen pendientes |
| [23_MATRIZ_DE_CARGOS.md](23_MATRIZ_DE_CARGOS.md) | Qué cargo exige cada ruta del motor: recepción, vendedor, operaciones o administrador, y las cuatro decisiones del 14-09 | Verificado | (documento transversal) | Aplicada en la rama `pruebas` el 14-09-2026, paso 2 del roadmap de venta; una prueba lee los decoradores y frena cualquier ruta nueva sin cargo |

Todos los mapas de este índice están, según su propia cabecera, "verificado una vez contra el código" al commit bd6a0e1 (11-09-2026). Si alguno cambia a "BORRADOR SIN VERIFICAR" en una revisión futura, esa fila debe marcarse como borrador aquí también.
...
## Las 16 zonas de mayor riesgo del sistema
...
16. **El catálogo con id correlativo, y las 12 tablas que se leían directo con la llave pública**: hasta el 11-09-2026 el catálogo (`service_groups`, `service_group_collections`, `fixed_services`) tenía id correlativo y varias rutas de escritura no comprobaban que la fila fuera de la empresa de la sesión — la puerta más fácil entre empresas, medida en producción el mismo día. El sprint 1 la cerró, pero **solo en el laboratorio** (rama `pruebas`, commit `8266ba1`): producción sigue abierta hasta que Felipe apruebe el paso a producción. Aparte, y esto sí ya resuelto: 12 tablas (`marketing_contacts`, `marketing_sends`, `marketing_suppressions`, `marketing_campaigns`, `marketing_audiences`, `quotation_followups`, `portal_receipts`, `fixed_service_sections`, `service_group_collection_fixed_services`, `consultas`, `consulta_config`, `event_types`) tenían la seguridad de fila apagada y permisos completos para `anon`/`authenticated` — se leían con la sola llave pública del bundle de www.eventi-app.com, sin pasar por el motor. Las migraciones 107, 108 y 109 las cerraron **en producción** el 11-09-2026; queda abierta la entrada de permisos por defecto a nombre de `supabase_admin` (en la práctica no aplica: las tablas de la casa las crea `postgres`) y que las funciones nuevas siguen naciendo con `EXECUTE` para `anon`. → [22_AISLAMIENTO_ENTRE_EMPRESAS.md](22_AISLAMIENTO_ENTRE_EMPRESAS.md), [05_CATALOGO_DE_SERVICIOS.md](05_CATALOGO_DE_SERVICIOS.md), `docs/migrations/107_cerrar_tablas_abiertas.sql`, `108_cerrar_el_grifo.sql`, `109_candado_get_backup_tables.sql`
...
- **Por eso la puerta entre empresas más fácil de recorrer era el catálogo, no la plata.** `DELETE /service-groups/:id` y `DELETE /service-group-collections/:id` (cargo ventas o más) borraban menús guardados y paquetes solo por id. `PATCH /services/fixed/:id` (solo administrador) cambiaba el precio de un servicio fijo solo por id. Una sesión de otra empresa podía hacerlo con datos ajenos. → [flujos/16_CAMBIAR_EL_CATALOGO.md](flujos/16_CAMBIAR_EL_CATALOGO.md), pasos 8, 23 y 24. **Esto ya tiene arreglo, pero solo en el laboratorio**: el sprint 1 de [22_AISLAMIENTO_ENTRE_EMPRESAS.md](22_AISLAMIENTO_ENTRE_EMPRESAS.md) (rama `pruebas`, commit `8266ba1`, 16 pruebas nuevas) filtra estas rutas y las de logística por `company_id` y devuelve 404 si la fila no era de la empresa. **No está en producción**: espera la validación de Felipe y su "a producción".
- **Cuántas empresas hay.** Producción tiene 3 empresas con datos: el aislamiento ya no es teórico.
- **Aparte de las rutas del motor, 12 tablas se leían directo con la llave pública, sin pasar por ninguna ruta.** `marketing_contacts`, `marketing_sends`, `marketing_suppressions`, `marketing_campaigns`, `marketing_audiences`, `quotation_followups`, `portal_receipts`, `fixed_service_sections`, `service_group_collection_fixed_services`, `consultas`, `consulta_config` y `event_types` tenían la seguridad de fila apagada y permisos completos (`SELECT, INSERT, UPDATE, DELETE, TRUNCATE`) para `anon`/`authenticated`: con la llave pública que viaja en el bundle de www.eventi-app.com se leían 2.150 contactos de marketing, 3.501 envíos y 203 notas comerciales (medido con conteos). **Esto sí se cerró en producción, el mismo 11-09-2026**, con las migraciones `107_cerrar_tablas_abiertas.sql`, `108_cerrar_el_grifo.sql` y `109_candado_get_backup_tables.sql`: las 12 tablas y la función `get_backup_tables()` responden 401 a la llave pública, el motor no se reinició, los relojes siguieron en 200 y el asesor de seguridad de Supabase quedó sin ningún ERROR. Queda abierta la entrada de permisos por defecto a nombre de `supabase_admin` (en la práctica no aplica: las tablas de la casa las crea `postgres`) y que las funciones nuevas siguen naciendo con `EXECUTE` para `anon`.

En producción, hoy 11-09-2026, siguen sin arreglo la cascada de plata (zona 2), la cotización pública (zona 3) y el catálogo (arreglado solo en el laboratorio, zona 16). Ya no está sin arreglo el acceso directo a las 12 tablas con la llave pública ni la función de respaldo: eso lo cerraron las migraciones 107-109. Cada arreglo que falta necesita plan y OK de Felipe.
