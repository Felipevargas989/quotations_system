...
| `GET /quotations/:id` | `QuotationsController.findOne` | `QuotationsService.findOne` → `QuotationsRepository.findOne` | `getQuotationById` ← `QuotationForm` (al editar, al duplicar y al releer la cotización recién guardada). La misma función la llaman además `NegocioPage`, `QuotationsPage`, `PostVentaPage`, `ClientDetailPage`, `Calendar`, `ResumenDelDia`, `PublicSurvey` y el componente `PaymentPlanEditor` | **`@Public()`** (lo usa la encuesta pública; tiene un TODO para crear otra puerta) |
...
| `DELETE /quotations/:id` | `QuotationsController.remove` | `QuotationsService.remove` (candado del realizado) → `QuotationsRepository.remove` → `assertDeletable` (privado) | `deleteQuotation` ← `QuotationForm` (botón Eliminar; la pantalla lo muestra solo a administrador) y `RequestsPage` (borrar un requerimiento) | Autenticado, **sin `@Roles`** en el motor |
| `GET /quotations/check-conflicts` | `QuotationsController.checkConflictsWithExistingQuotations` | `QuotationsService.checkConflictsWithExistingQuotations` | `checkConflictsWithExistingQuotations` vía el hook `useDateAvailability` ← `QuotationForm` y `RequestForm` (montado en `RequestsPage`); también directo desde el componente `EventoCajitas`, que vive en `NegocioPage` y `PostVentaPage` | Autenticado |
...
| Método y ruta | Controller y método | Service | Quién lo llama desde la app | Roles o @Public |
|---|---|---|---|---|
| `GET /services` | `ServicesController.findAll` | services | `findAllServices` vía el hook `useServices` ← `QuotationForm`, `ServicesPage` y `ServiciosTab` (montado en `NegocioPage` y `PostVentaPage`); directo también desde `NewAccount`, la tarjeta de cuenta nueva de `DashboardPage` | Autenticado, sin `@Roles` (ver 05) |
| `GET /services/fixed-sections` | `ServicesController.listFixedSections` | services | `getFixedSections` (query `["fixedSections"]`) ← `QuotationForm`, `ServiciosTab` (en `NegocioPage` y `PostVentaPage`) y `FixedServicesBySection` (en `ServicesPage`) | Autenticado, sin `@Roles` |
| `GET /sections` | `sections.controller.ts` (`@Controller('sections')`) | sections | `getCategorySections` ← `QuotationForm`, `ServiciosTab` (en `NegocioPage` y `PostVentaPage`) y `VariableServicesByCategory` (en `ServicesPage`) | `RECEPTION_AND_UP` |
| `GET/POST/PATCH/DELETE /service-groups` | `service-groups.controller.ts` (`findAll`, `create`, `rename`, `remove`) | service-groups | hook `useServiceGroups` (`saveGroup`, `renameGroup`, `removeGroup`) ← `QuotationForm`, la única pantalla que crea, renombra y borra menús; `ServiciosTab` (en `NegocioPage` y `PostVentaPage`) solo usa la lista | GET sin `@Roles`; escritura `SALES_AND_UP` |
| `GET/POST/DELETE /service-group-collections` | `service-group-collections.controller.ts` | service-group-collections | hook `useServiceGroupCollections` ← `QuotationForm`, y ninguna otra pantalla | GET sin `@Roles`; escritura `SALES_AND_UP` |
| `GET /logistics/base-catalogo` | `LogisticsController.baseCatalogo` | logistics | `getBaseCatalogo` ← `QuotationForm`, para el cuadro Margen y costos (solo lo pide con rol admin u operaciones). La misma función la llaman `DashboardPage` y `ComprasTab` (en `LogisticaPage`), y por el hook `useBaseLogistica` las pestañas `ServiciosTab`, `CocinaTab` y `GestionTab` (en `PostVentaPage`) y `MobiliarioTab` (en `LogisticaPage`) | `SALES_AND_UP` (ver 06) |
| `GET /event-types` y `GET /event-types/public/:companyId` | `event-types.controller.ts` (`listar`, `listarPublico`) | consultas | `eventTypesQueryOptions` ← `QuotationForm` y `RequestForm` (en `RequestsPage`); `getEventTypesPublic` ← `CreateQuotationPublic`, el formulario público | Autenticado / `@Public` + throttle 30 por minuto (ver 11) |
| `GET/POST /client-contacts`, `POST /clients`, `GET /clients` | controllers de clientes | clients | `getClientContacts` ← `QuotationForm` y `RequestForm` (en `RequestsPage`); `createClientContact` ← `QuotationForm` y `ClientDetailPage`; `createClient` y `clientsQueryOptions` ← `QuotationForm`, `RequestForm` (en `RequestsPage`) y `ClientsPage` | ver 09 |
| `GET /payments?quotationId=` | payments | payments | `getPaymentsByQuotationId` dentro de `AvisoPlanDePagos` ← `QuotationForm` y `ServiciosTab` (en `NegocioPage` y `PostVentaPage`), las dos pantallas que montan ese aviso. La misma función la llaman directo `NegocioPage` y `QuotationsPage` | ver 03 |
